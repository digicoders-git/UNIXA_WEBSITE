...
const ProductCard = memo(({ product }) => {
    ...
    return (
        <div
            ...
            className="group bg-white rounded-[1.5rem] md:rounded-[2rem] border border-slate-300/50 p-3 md:p-5 transition-all duration-500 hover:shadow-xl hover:-translate-y-1 flex flex-col h-full cursor-pointer relative overflow-hidden"
        >
            {/* Discount Badge */}
            {discount > 0 && (
            ...
            {/* Image Container - Reduced padding and aspect for shorter cards */}
            ...
            <div className="flex gap-2 w-full mt-auto" onClick={e => e.stopPropagation()}>
                ...
                <button
                    ...
                    className="flex-[1.5] py-2 md:py-3 bg-[var(--color-secondary)] text-white rounded-xl font-black text-[8px] md:text-[9px] uppercase tracking-widest hover:bg-[var(--color-primary)] shadow-md transition-all active:scale-95 flex items-center justify-center gap-1"
                >
                    Buy Now <ArrowRight size={10} className="md:w-3 md:h-3" />
                </button>
            </div>
        </div>
    );
